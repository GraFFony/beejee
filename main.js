...
async function load_script() {
    res = await fetch(`/PHP/TasksController.php?page=${page}`, {
        method: "GET",
    }).then(a => a.json());
    ...
    if (await res) {
        ...
        for (i = 0; i < res[0].length; i++) {
            ...
            if (user !== '3') {
                ...
                div2.classList = "card-text card_need_info";
                if (res[0][i]['work'] === "0")
                div2.innerHTML = '<select id="selector'+res[0][i]['id']+'" class="form-control" onchange="changeTask('+res[0][i]['id']+')"> <option value="0" selected>не выполняется</option><option value="1">выполняется</option></select>';
                if (res[0][i]['work'] === "1")
                    div2.innerHTML = '<select id="selector'+res[0][i]['id']+'" class="form-control" onchange="changeTask('+res[0][i]['id']+')"> <option value="0" >не выполняется</option><option value="1" selected>выполняется</option></select>';
                div.appendChild(div2);
            }
        }
        if (res[1] % 3 !== 0) {
...
async function log_in() {
    ...
    if ($('#login').value !== '' && $('#password').value !== '') {
        ...
        if (user === '3') {
            alert('вы успешно вошли в аккаунт администратора!');
